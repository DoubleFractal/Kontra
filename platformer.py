...
class Player(Object):
    def __init__(self):
        ...
        self.base = pygame.image.load(os.path.join("data", "gunner.png"))
        self.image = self.base.copy()
        self.rect = self.image.get_rect(midtop=(64, 8))
        self.head_pics = [
            pygame.image.load(os.path.join("data", x))
            for x in ["head-normal.png", "head-upright.png", "head-downright.png"]
        ]
    ...
    def collide_with_floors(self):
        for platform in self.platforms:
            if (
                self.fall_through
                and platform.allow_fall_through
                and self.jump_speed > 3
            ):
                continue
...
class NPC(Player):
    def __init__(self):
        ...
        self.base = pygame.image.load(os.path.join("data", "alien.png"))
        self.image = self.base
...
class Helicopter(NPC):
    def __init__(self, pos):
        ...
        self.frames = [
            pygame.image.load(os.path.join("data", "helicopter-1.png")),
            pygame.image.load(os.path.join("data", "helicopter-2.png")),
        ]
    ...
    def onscreen(self, camera):
        if (
            camera.translate(
                pygame.Rect(
                    [self.startx, self.rect.y, self.rect.width, self.rect.height]
                )
            ).x
            < 100
        ):
...
class Bomb(Bullet):
    def __init__(self, pos, direction):
        ...
        self.image = pygame.image.load(os.path.join("data", "bomb.png"))
        self.rect = self.image.get_rect(center=pos)
        self.direction = direction
        self.z = -1
        self.damage = "player"
